...
// добавлено  23.08.2018 CatMan

$(function(){
    var $bx_pvz = $('#boxberry_pvz');
    $bx_pvz.change(function() {
        $("#boxberry_change").addClass('noDisplay');
        $('#newCarrierPrice').html('');
        var orderData = $bx_pvz.data('order'),
            $selectedOpt = $bx_pvz.find('option:selected'),
            price = orderData.price,
            code = $selectedOpt.val(),
            address = $selectedOpt.text();
        var params = {
           'boxberry_calc' : true,
           'price' : price,
           'code' : code,
       ...
       $.ajax({
            ...
            success: function(res) {
                var $newCarrierPrice = $('#newCarrierPrice');
                var resObj = $.parseJSON(res);
                var newPrice = resObj.price;
		$newCarrierPrice.html(newPrice);
                $("#boxberry_change").removeClass('noDisplay');
            }
        });
    });
