	...
	<table id="carrierTable" class="std" {if !isset($carriers) || !$carriers || !count($carriers)}style="display:none;"{/if}>
		...
		{if isset($carriers)}
			{foreach from=$carriers item=carrier name=myLoop}
				<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}{if $carrier.external_module_name} {$carrier.external_module_name}_block{/if}">
					<td class="carrier_action radio">
						<input {if $carrier.external_module_name}class="{$carrier.external_module_name}" {/if}type="radio" name="id_carrier" value="{$carrier.id_carrier|intval}" id="id_carrier{$carrier.id_carrier|intval}"  {if $opc}onclick="updateCarrierSelectionAndGift();"{/if} {if !($carrier.is_module AND $opc AND !$isLogged)}{if $carrier.id_carrier == $checked}checked="checked"{/if}{else}disabled="disabled"{/if} />
					</td>
					...
					<td class="carrier_price">
                        ...
                        {elseif $carrier.carrier_text neq ''}
                            {$carrier.carrier_text|escape:'htmlall':'UTF-8'}
						{else}
							{l s='Free!'}
						{/if}
					</td>
				...
				{if $carrier.external_module_name}
					...
					<tr><td colspan="4">
            <div class="bx_container{if $carrier.id_carrier != $checked} hide{/if}" data-price="{$price_for_boxberry}">	
...
var afterSelectPoint = function(price,address) {
    ...
    $btnNext.removeAttr('disabled');
                ...
                })},'{$boxberry_widget_config.token}',
                  '{$boxberry_widget_config.city}',
                  '{$boxberry_widget_config.target_start}',
                  '{$boxberry_widget_config.price}',
                  '{$boxberry_widget_config.weight}',
                  '{$boxberry_widget_config.paysum}',
                  '{$boxberry_widget_config.height}',
                  '{$boxberry_widget_config.width}',
                  '{$boxberry_widget_config.depth}')">Выбрать ПВЗ</a>             
