...
{if !isset($dlv_all_fields)}
        ...
        {$dlv_all_fields.3 = 'middlename'}
		{$dlv_all_fields.4 = 'address1'}
		{$dlv_all_fields.5 = 'address2'}
		{$dlv_all_fields.6 = 'postcode'}
		{$dlv_all_fields.7 = 'city'}
		{$dlv_all_fields.8 = 'country'}
		{$dlv_all_fields.9 = 'state'}
{/if}
			...
			<fieldset>
				...
				<div id="opc_account_form" style="display: block; ">
                    ...
                    <p class="required text">
						...
						<input type="text" class="text" id="middlename" name="middlename" onblur="$('#customer_middlename').val($(this).val());" value="{if isset($smarty.post.middlename)}{$smarty.post.middlename}{/if}">
						<input type="hidden" class="text" id="customer_middlename" name="customer_middlename" value="{if isset($smarty.post.middlename)}{$smarty.post.middlename}{/if}">
						<sup>*</sup>
					</p>
					<p class="select">
...
<form action="{$link->getPageLink('authentication.php', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
		...
		<p class="required text">
			...
			<input style="text-transform: capitalize;" onkeyup="$('#firstname').val(this.value);" type="text" class="text" id="customer_firstname" name="customer_firstname" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}" />
			<sup>*</sup>
		</p>
		<p class="required text hide">
			<label for="customer_lastname">{l s='Last name'}</label>
			<input style="text-transform: capitalize;" type="text" class="text" id="customer_lastname" name="customer_lastname" value="-" />
			<sup>*</sup>
		</p>
        <p class="required text hide">
			<label for="customer_middlename">{l s='Middle name'}</label>
			<input  style="text-transform: capitalize;" type="text" class="text" id="customer_middlename" name="customer_middlename" value="-" />
			<sup>*</sup>
		</p>
		<p class="required text">
		...
		<p class="hide" style="margin-left:20%px;">{l s='You must register at least one phone number'} <sup style="color:red;">*</sup></p>
		<p class="hide" class="text">
			<label for="phone">{l s='Home phone'}</label>
			<input type="text" class="text" name="phone_mobile" id="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}" />
		</p>
		<p class="required text">
			<label for="phone_mobile">Контактный телефон</label>
			<input type="text" class="text" name="phone" id="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
                        <sup>*</sup>
		</p>
		<p class="select hide">
			<span>{l s='Date of Birth'}</span>
        ...
        <span style="margin-left: 30px;"><sup style="color: red;">*</sup>{l s='Required field'}</span>
       <p class="checkbox" style="margin-left: -90px;margin-top: 20px;" >
			<input type="checkbox" name="visible_adress" id="vadress" value="1"/>
			<label for="vadress" style="font-size: 14px; "><strong>{l s='Ввести адрес доставки(не требуется при самовывозе)'}</strong></label>
	   </p>
 ...
 {literal}
$(document).ready(function() {
$("#vadress").change(function(){ 
if($("#vadress").prop("checked")){
    $('#acreation').show();      
}else{
    $('#acreation').hide(); 
}
...
{/literal}
	...
	<fieldset class="account_creation" id="acreation" style="display: none;">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "firstname"}
				<p class="required text">
					<label for="firstname">{l s='First name'}</label>
					<input type="text" style="text-transform: capitalize;" class="text" id="firstname" name="firstname" value="{if isset($smarty.post.firstname)}{$smarty.post.firstname}{/if}" />
					<sup>*</sup>
				</p>
			{elseif $field_name eq "lastname"}
				<p class="required text">
					<label for="lastname">{l s='Last name'}</label>
					<input type="text" style="text-transform: capitalize;" class="text" id="lastname" name="lastname" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" />
					<sup>*</sup>
				</p>
            {elseif $field_name eq "middlename"}
				<p class="required text">
					<label for="middlename">{l s='Middle name'}</label>
					<input type="text" style="text-transform: capitalize;" class="text" id="middlename" name="middlename" value="{if isset($smarty.post.middlename)}{$smarty.post.middlename}{/if}" />
					<sup>*</sup>
				</p>
			{elseif $field_name eq "address1"}
				<p class="required text">
