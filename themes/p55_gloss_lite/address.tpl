...
<form action="{$link->getPageLink('address.php', true)}" method="post" class="std">
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		{/if}
        {if $field_name eq "middlename"}
        <p class="required text">
            <label for="middlename">{l s='Middle name'}</label>
            <input type="text" class="text" id="middlename" name="middlename" value="{if isset($smarty.post.middlename)}{$smarty.post.middlename}{else}{if isset($address->middlename)}{$address->middlename|escape:'htmlall':'UTF-8'}{/if}{/if}" />
            <sup>*</sup>
        </p>
        {/if}
		...
		<p style="margin-left:20%px;">{l s='You must register at least one phone number'} <sup style="color:red;">*</sup></p>
		<p class="text">
