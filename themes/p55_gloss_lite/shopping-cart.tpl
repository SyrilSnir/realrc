	...
	<form action="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}" method="post" id="voucher">
		<fieldset>
			...
			<div>
				...
				<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='Add'}" class="button_small" /></p>
			</div>
		</fieldset>
	</form>
</div>
...
{if !isset($addresses_style)}
    ...
    {$addresses_style.middlename = 'address_name'}
...
<div class="order_delivery" {if !$delivery->id}  style="display:none" {/if}  >
	{if !isset($formattedAddresses)}
		...
		<ul id="delivery_address" class="address item">
			<li class="address_title">{l s='Delivery address'}</li>
				...
				<li class="address_name">{$delivery->firstname|escape:'htmlall':'UTF-8'} {$delivery->lastname|escape:'htmlall':'UTF-8'} {$delivery->middlename|escape:'htmlall':'UTF-8'}</li>
		...
		<ul id="invoice_address" class="address alternate_item">
			<li class="address_title">{l s='Invoice address'}</li>
				...
				<li class="address_name">{$invoice->firstname|escape:'htmlall':'UTF-8'} {$invoice->lastname|escape:'htmlall':'UTF-8'} {$delivery->middlename|escape:'htmlall':'UTF-8'}</li>
