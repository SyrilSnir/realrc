...
{if !$refresh}
	...
	<p id="permalink"><span>Permalink:</span> <input type="text" value="{$base_dir_ssl}modules/blockwishlist/view.php?token={$token_wish|escape:'htmlall':'UTF-8'}" style="width: 460px; float: right; margin-right: 7px;" readonly/></p>
{/if}
	...
	<ul class="address {if $smarty.foreach.i.index % 2}alternate_{/if}item" style="margin:5px 0 0 5px;border-bottom:1px solid #ccc;" id="wlp_{$product.id_product}_{$product.id_product_attribute}">
		<li class="item">
			<a class="image-holder" href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)}" title="{l s='Product detail' mod='blockwishlist'}">
				<img src="{$link->getImageLink($product.link_rewrite, $product.cover, 'medium')}" alt="{$product.name|escape:'htmlall':'UTF-8'}" />
			</a>
		<div class="wishlist_product_detail">
       <p>{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}</p>
		...
		{/if}
      <p><span>{l s='Quantity' mod='blockwishlist'}:</span> <input type="text" id="quantity_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity|intval}" size="3"  /></p>
      <p>
        <span>{l s='Priority' mod='blockwishlist'}:</span> 
        <select id="priority_{$product.id_product}_{$product.id_product_attribute}">
          ...
          <option value="2"{if $product.priority eq 2} selected="selected"{/if}>{l s='Low' mod='blockwishlist'}</option>
			...
			<a href="javascript:;" class="delete-button button" onclick="WishlistProductManage('wlp_bought', 'delete', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='Delete' mod='blockwishlist'}">{l s='Delete' mod='blockwishlist'}</a>
			<a href="javascript:;" class="save-button exclusive" onclick="WishlistProductManage('wlp_bought_{$product.id_product_attribute}', 'update', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='Save' mod='blockwishlist'}">{l s='Save' mod='blockwishlist'}</a>
		</li>
	</ul>
	{/foreach}
	</div>
	...
	{/if}
{else}
	<p class="notification-empty">{l s='No products' mod='blockwishlist'}</p>
{/if}
