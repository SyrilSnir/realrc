...
class SdiscountCore extends ObjectModel
{
    ...
    public function getHistory($id_lang,$id_order,$id_order_state = false, $no_hidden = false, $filters = 0)
	{
		if (!$id_order_state)
			$id_order_state = 0;
   ...
   public static function getSdiscountInfo($id_customer){
        $groupId = Db::getInstance()->ExecuteS('
		SELECT id_customer
		FROM `'._DB_PREFIX_.'customer_group` WHERE id_customer='.$id_customer.' AND id_group !=1');
        if (count($groupId) > 0) return 0;
        global $cookie;          
        $counPrice = self::getSum($id_customer);     
        $sdicountStats = Db::getInstance()->ExecuteS('
		SELECT *
		FROM `'._DB_PREFIX_.'sdiscount`
		ORDER BY `sum_sdiscount` ASC');
        //echo "<pre>";
        //print_r($sdicountStats);
        //echo "</pre>";
        
        foreach($sdicountStats as $stats) {
	         if($stats['sum_sdiscount']<$counPrice){
	 
               $min = $stats['sum_sdiscount'];
               $minStats = $stats['id_sdiscount'];
       ...
       if(isset($min)){
        ...
        $sdicountStats = array_reverse($sdicountStats);
        foreach($sdicountStats as $stats) {
	         if($stats['sum_sdiscount']>$counPrice){
	 
               $max = $stats['sum_sdiscount'];
               $maxStats = $stats['id_sdiscount'];
               
	         }
        }
        $leftOrdered = $max - $counPrice; 
        }else{
            $maxStats = 0;
            $leftOrdered = 0;
            
        }
        $realStatus = array('real' =>$minStats,
                            'next' =>$maxStats,
   ...
   public static function getPercent($summ,$id_dicount)
   {
     $discount = self::getSdicount($id_dicount);
     
     $summPercent =  ($summ / 100) * $discount['percent_sdiscount'];
     
     return $summPercent;
    
   }
    
   public static function getSdicount($id_dicount)
   {
      if($id_dicount > 0){
       $sdicountStats = Db::getInstance()->ExecuteS('
		SELECT *
		FROM `'._DB_PREFIX_.'sdiscount`
		WHERE  id_sdiscount = "'.$id_dicount.'"');

     return $sdicountStats[0];
     }
   ...
   public static function getSum($id_customer)
   {
        ...
        foreach ($orderReceived as $items) {
	         $statLast  = self::getHistory($cookie->id_lang,$items['id_order']);
