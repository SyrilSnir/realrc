...
class OrderHistoryCore extends ObjectModel
{
	...
	public function addWithemail($autodate = true, $templateVars = false)
	{
		...
		SELECT osl.`template`, c.`lastname`, c.`firstname`, c.`middlename`, osl.`name` osname, c.`email`
		...
		{
			$topic = $result['osname'];
			$data = array('{lastname}' => $result['lastname'], '{firstname}' => $result['firstname'], '{middlename}' => $result['middlename'], '{id_order}' => (int)$this->id_order);
			if ($templateVars)
			...
			if (($virtualProducts = $order->getVirtualProducts()) && (!$lastOrderState || !$lastOrderState->logable) && ($newOrderState = new OrderState($this->id_order_state, _PS_LANG_DEFAULT_)) && $newOrderState->logable)
			{
				...
				Mail::Send((int)$order->id_lang, 'download_product', Mail::l('Virtual product to download', (int)$order->id_lang), $data, $result['email'], $result['firstname'].' '.$result['middlename'].' '.$result['lastname']);
			}

			if (Validate::isLoadedObject($order))
				Mail::Send((int)$order->id_lang, $result['template'], $topic, $data, $result['email'], $result['firstname'].' '.$result['middlename'].' '.$result['lastname']);
		}
