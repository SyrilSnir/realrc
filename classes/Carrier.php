...
class CarrierCore extends ObjectModel
{
	...
	const SHIPPING_METHOD_EXTERNAL = 4;
	...
	/** @var mixed Delay needed to deliver customer */
	public $delay;
	...
	public function getFields()
	{
		...
		$fields['carrier_text'] = (string)($this->carrier_text);
		...
		$fields['need_range'] = (int)$this->need_range;
 	    $fields['is_adress'] = (int)$this->is_adress;
	...
	*/
	...
	{       
                $this->position = Carrier::getLastPosition();
	...
	*/
    ...
    public function getCarierIds($value)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS("SELECT * FROM `"._DB_PREFIX_."carrier` WHERE `id_carrier` = $value");
        $name =  $result[0]['name'];
       	
        $result_name = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS("SELECT * FROM `"._DB_PREFIX_."carrier` WHERE `name` = '".$name."'");
        
        $ids = array();
        foreach($result_name as $items){
            $ids[] = $items['id_carrier'];
        }
	...
	public static function getDeliveredCountries($id_lang, $activeCountries = false, $activeCarriers = false, $containStates = null, $activeStates = false)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			SELECT cl.*,c.*, cl.`name` country, zz.`name` zone FROM `'._DB_PREFIX_.'country` c 
	...
	public static function getCarriersForOrder($id_zone, $groups = null)
	{
		...
		if (is_array($groups) && !empty($groups))
			$result = Carrier::getCarriers((int)$cookie->id_lang, true, false, (int)$id_zone, $groups, self::ALL_CARRIERS);
		else
			$result = Carrier::getCarriers((int)$cookie->id_lang, true, false, (int)$id_zone, array(1), self::ALL_CARRIERS);
		$resultsArray = array();
		...
		foreach ($result as $k => $row)
		{
			...
			$shippingMethod = $carrier->getShippingMethod();
                if ( ($shippingMethod != Carrier::SHIPPING_METHOD_FREE) && ($shippingMethod != Carrier::SHIPPING_METHOD_EXTERNAL ) )
			{
			...
			$row['price'] = ( ($shippingMethod == Carrier::SHIPPING_METHOD_FREE || $shippingMethod == Carrier::SHIPPING_METHOD_EXTERNAL) ? 0 : $cart->getOrderShippingCost((int)$row['id_carrier']));
	...
	public function deleteDeliveryPrice($rangeTable)
	{
		return Db::getInstance()->Execute('
			...
			WHERE `id_carrier` = '.(int)($this->id).'
			AND (`id_'.$rangeTable.'` IS NOT NULL
			OR `id_'.$rangeTable.'` = 0)
		');
	}
	...
	public function copyCarrierData($oldId)
	{
		...
		foreach ($res as $val)
		{
			...
			if (count($res2))
			{
				...
				foreach ($res2 as $val2)
					$sql .= '('.(int)($this->id).','.(int)($maxRangePrice).',NULL,'.(int)($val2['id_zone']).','.(float)($val2['price']).'),';
				$sql = rtrim($sql, ',');
		...
		foreach ($res as $val)
		{
			...
			if (count($res2))
			{
				$sql = 'INSERT INTO `'._DB_PREFIX_.'delivery` (`id_carrier`,`id_range_price`,`id_range_weight`,`id_zone`, `price`) VALUES';
				foreach ($res2 as $val2)
					$sql .= '('.(int)($this->id).',NULL,'.(int)($maxRangeWeight).','.(int)($val2['id_zone']).','.(float)($val2['price']).'),';
				$sql = rtrim($sql, ',');
				Db::getInstance()->Execute($sql);
			}
		}
		...
		if ((int)$oldId != (int)$this->id && count($res))
		{
			$sql = 'INSERT INTO `'._DB_PREFIX_.'carrier_zone` (`id_carrier`, `id_zone`) VALUES';
			foreach ($res as $val)
				$sql .= '('.(int)$this->id.','.(int)$val['id_zone'].'),';
			$sql = rtrim($sql, ',');
			Db::getInstance()->Execute($sql);
		}
	...
	**/
	...
	public static function getIdTaxRulesGroupByIdCarrier($id_carrier)
	{
        ...
        public function updatePosition ($way,$position) 
        {
            if (!$res = Db::getInstance()->ExecuteS('
                SELECT cp.`id_carrier`, cp.`position`
                FROM `'._DB_PREFIX_.'carrier` cp
                WHERE cp.`id_carrier` = '.(int)$this->id)) {
                return false;
            }
            $id_carrier = $res[0]['id_carrier'];
            $cur_position = $res[0]['position'];
            
            $result = (Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'carrier`
			SET `position`= `position` '.($way ? '- 1' : '+ 1').'
                        WHERE `position`'.($way ? '> '.$cur_position. ' AND `position` <= ' . (int) $position :
                         '< '.$cur_position. ' AND `position` >= ' . (int) $position ))
                    AND
	...
	{
		return Db::getInstance()->getValue('SELECT MAX(position) + 1 FROM `'._DB_PREFIX_.'carrier` WHERE `deleted` = 0');
	}
}
