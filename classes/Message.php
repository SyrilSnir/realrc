...
class MessageCore extends ObjectModel
{
	...
	public static function getMessagesByOrderId($id_order, $private = false)
	{
		...
		SELECT m.*, c.`firstname` cfirstname, c.`lastname` clastname, c.`middlename` cmiddlename, e.`firstname` efirstname, e.`lastname` elastname, (COUNT(mr.id_message) = 0 AND m.id_customer != 0) is_new_for_me
