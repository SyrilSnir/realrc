  ...
  //          else if ($from != $to)
   //             $adrForReply = $from;
	...
	//	if (!isset($from) || !Validate::isEmail($from))
		$fromSend = $configuration['PS_SHOP_EMAIL'];
...
// Было $to_list->addTo($addr, '=?UTF-8?B?'.base64_encode($to_name).'?=');
			...
			if ($toName == null)
				$toName = $to;
            /* Encode accentuated chars */
// Было                        $to = new Swift_Address($to, '=?UTF-8?B?'.base64_encode($toName).'?=');
		...
		try {
			...
			$message = new Swift_Message('['.Configuration::get('PS_SHOP_NAME').'] '.$subject);
                       ...
                       //if (is_a($to,'Swift_Address'))
                       // {
                       // $controlAddress = $to->getAddress();
			
                        
                   //     if (isset($adrForReply) && $adrForReply != null && $from != $adrForReply)
                        if ($from != $fromSend && $from )
                        {
                     ...
                     //   if ($message->getFrom() == '')
                      //  else
                        //    $message->setFrom ($fromName. ' <'.$fromSend . '>');
			...
			/* Send mail */
               //         $to->setName("Обслуживание покупателей");
                        
			$send = $swift->send($message, $to, new Swift_Address($fromSend, $fromName));
			$swift->disconnect();
			return $send;
                //        return 1;
		}
