...
class PDFCore extends PDF_PageGroupCore
{
	...
	public static function invoice($order, $mode = 'D', $multiple = false, &$pdf = null, $slip = false, $delivery = false)
	{
		...
		if (!self::$delivery)
		{
			...
			{ 
			    ...
			    $pdf->Cell($width, 0, '—кидка('.$statusUserDisc['name_sdiscount'].' - '.$statusUserDisc['percent_sdiscount'].'%)  : ', 0, 0, 'R');
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '-').self::convertSign(Tools::displayPrice(($minusSumm), self::$currency, true)), 0, 0, 'R');
				$pdf->Ln(4);
                }
                
				$pdf->Cell($width, 0, self::l('Total').' : ', 0, 0, 'R');
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(($order->total_paid), self::$currency, true)), 0, 0, 'R');
				$pdf->Ln(4);
			}
	...
	*/
	...
	{
		$s = str_replace(array('И', '?', '?'), array(chr(128), chr(163), chr(165)), $s);
