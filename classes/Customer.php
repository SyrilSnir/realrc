...
class CustomerCore extends ObjectModel
{
	...
	protected $tables = array ('customer');
 	...
 	protected $fieldsRequired = array('lastname', 'passwd', 'firstname', 'email', 'middlename');
 	protected $fieldsSize = array('lastname' => 32,'phone' => 16, 'passwd' => 32, 'firstname' => 32, 'middlename' => 32,'email' => 128, 'note' => 65000);
 	protected $fieldsValidate = array('secure_key' => 'isMd5','phone' => 'isPhoneNumber', 'lastname' => 'isName', 'firstname' =>'isName', 'middlename' => 'isName',  'email' => 'isEmail', 'passwd' => 'isPasswd',
	...
	public static function getNewsletteremails()
	{
		Tools::displayAsDeprecated();
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT `email`, `firstname`, `lastname`, `middlename`, `newsletter`, `ip_registration_newsletter`, `newsletter_date_add`
		FROM `'._DB_PREFIX_.'customer`
	...
	public static function searchByName($query)
	{
		...
		WHERE ( a.`phone_mobile` LIKE \'%'.pSQL($query).'%\' OR a.`phone` LIKE \'%'.pSQL($query).'%\') AND a.`id_customer` = c.`id_customer`
                UNION SELECT c1.* 
                FROM `'._DB_PREFIX_.'customer` c1
                WHERE c1.`email` LIKE \'%'.pSQL($query).'%\' OR c1.`id_customer` LIKE \'%'.pSQL($query).'%\'
		OR c1.`lastname` LIKE \'%'.pSQL($query).'%\' OR c1.`firstname` LIKE \'%'.pSQL($query).'%\' OR c1.`middlename` LIKE \'%'.pSQL($query).'%\'
                OR c1.`phone` LIKE \'%'.pSQL($query).'%\'');
	}
	...
	*/
	...
	public function transformToCustomer($id_lang, $password = null)
	{
		...
		if ($this->update())
		{
			$vars = array('{firstname}' => $this->firstname, '{lastname}' => $this->lastname, '{middlename}' => $this->middlename, '{email}' => $this->email, '{passwd}' => $password);
			Mail::Send((int)$id_lang, 'guest_to_customer', Mail::l('Your guest account has been transformed to customer account', (int)$id_lang), $vars, $this->email, $this->firstname.' '.$this->middlename.' '.$this->lastname);
			return true;
		}
		return false;
	}
