		...
		{ 
          ...
          if ($customer->id_default_group == '1')
           $sdiscount = Sdiscount::getSdiscountInfo((int)($cart->id_customer));
            ...
            //Если есть скидка отнимает от наценки сумму скидки
             if($order->total_discounts>0 and !is_array($sdiscount)){
                  
             //Считаем остаток по скидке                
            $real_discount_temp = $cart->getDiscounts();
            $real_discount = 0;            
            foreach($real_discount_temp as $items){                
                $real_discount += $items['value_real'];
            }                                      
               $price_discount_total = ($order->total_products+$order->total_shipping)- $real_discount;               
               if($price_discount_total<0){
                    $order->residue_discount = abs($price_discount_total); 
               }
                    
                
                
                (float)$order->total_products_markup  = (float)($cart->total_products_markup) - $order->total_discounts;
            
            }else{
                $order->total_products_markup = (float)($cart->total_products_markup);
            }
            
             
            if(is_array($sdiscount)){            
              $minusSumm = Sdiscount::getPercent($order->total_products_wt,$sdiscount['real']); 
              $order->total_paid  = ($order->total_paid - $order->total_shipping) - $minusSumm;
              $order->total_paid  = $order->total_paid + $order->total_shipping;
              
              $order->total_paid_real  = ($order->total_paid_real - $order->total_shipping) - $minusSumm;
              $order->total_paid_real  = $order->total_paid_real + $order->total_shipping;
              
              $order->total_products_markup = $order->total_products_markup - $minusSumm;
              $order->sdiscount = $sdiscount['real'];
			...
			{ 
                            // проверка на наличие модулей для ПВЗ
                            $carrier = new Carrier($order->id_carrier);
                            if (($carrier->is_module == 1 ) && ( $carrier->shipping_external == 1 ) && !empty($carrier->external_module_name)  ) {
                                $moduleName = $carrier->external_module_name;
                                $module = Module::getInstanceByName($moduleName);
                                if (Validate::isLoadedObject($module)) {
                                    if (method_exists($module, 'saveShippingData')) {
                                        Sessions::sessionInit();
                                        $pvz_props = Sessions::getVarSession('SELECTED_PVZ');
                                        $module->saveShippingData( $pvz_props,$order->id);
                                    } 
                                }
                            }
				if (!$secure_key)
				...
				$query = 'INSERT INTO `'._DB_PREFIX_.'order_detail`
					(`id_order`, `product_id`, `product_attribute_id`, `product_name`, `product_quantity`, `product_quantity_in_stock`, `product_price`, `product_markup`, `reduction_percent`, `reduction_amount`, `group_reduction`, `product_quantity_discount`, `product_ean13`, `product_upc`, `product_reference`, `product_supplier_reference`, `product_weight`, `tax_name`, `tax_rate`, `ecotax`, `ecotax_tax_rate`, `discount_quantity_applied`, `download_deadline`, `download_hash`)
				VALUES ';
				...
				foreach ($discounts as $discount)
				{
					...
					if ($shrunk AND ($total_discount_value + $value) > ($order->total_products_wt + $order->total_shipping + $order->total_wrapping))
					{
						...
						if ($objDiscount->id_discount_type == 2 AND $objDiscount->behavior_not_exhausted == 2)
						{
							...
							@Mail::Send((int)$order->id_lang, 'voucher', Mail::l('New voucher regarding your order #', (int)$order->id_lang).$order->id, $params, $customer->email, $customer->firstname.' '.$customer->middlename.' '.$customer->lastname);
						}
					}
					else
						$amount_to_add = $value;
				...
				$new_history->addWithemail(true, $extraVars);
                                ...
                                $total_discount = Tools::displayPrice($order->total_discounts, $currency, false);
                                $my_discount = "Скидка:";
                                $minusSumm = Tools::displayPrice($order->total_discounts, $currency, false);
                                
                                if(is_array($sdiscount) && $order->sdiscount >0)
                                {
                                    ...
                                    $my_discount .= ' ('.$statusUserDisc['name_sdiscount'].' – '.$statusUserDisc['percent_sdiscount'].'%)';
                                    $total_discount = Tools::displayPrice($minusSumm, $currency, false);
                                }
				...
				if ($id_order_state != Configuration::get('PS_OS_ERROR') AND $id_order_state != Configuration::get('PS_OS_CANCELED') AND $customer->id)
				{
					...
					$carrier = new CarrierCore((int)($order->id_carrier), $order->id_lang);
					$delivery_state = $delivery->id_state ? new State((int)($delivery->id_state)) : false;
					$invoice_state = $invoice->id_state ? new State((int)($invoice->id_state)) : false;
                                        $carrier_txt = Tools::displayPrice($order->total_shipping, $currency, false);
                                        if (!empty ($carrier->carrier_text))
                                        {
                                            if ($carrier->external_module_name == 'boxberry') {
                                                $bx_order_id = OrderBoxberryCore::getIdFromOrder($order->id);
                                                $orderBoxberry = new OrderBoxberryCore($bx_order_id);
                                                $carrier_txt = $orderBoxberry->shipping_price . ' руб';
                                            } else {
                                                $carrier_txt = 'Рассчитывается при отправке';
                                            }
                                        }
					...
					'{delivery_block_html}' => $this->_getFormatedAddress($delivery, "<br />",
						array(
                            ...
                            'middlename'	=> '<span style="color:#DB3484; font-weight:bold;">%s</span>',
                    ...
                    '{delivery_middlename}' => $delivery->middlename, 
                    ...
                    '{invoice_middlename}' => $invoice->middlename,
					...
					'{total_products}' => Tools::displayPrice($order->total_paid - $order->total_shipping - $order->total_wrapping + $order->total_discounts, $currency, false),
                                        '{my_discounts}' => $my_discount,
					'{total_discounts}' => $total_discount,
					'{total_shipping}' => $carrier_txt,
					...
					if (Validate::isEmail($customer->email))
						Mail::Send((int)$order->id_lang, 'order_conf', Mail::l('Order confirmation', (int)$order->id_lang), $data, $customer->email, $customer->firstname.' '.$customer->middlename.' '.$customer->lastname, NULL, NULL, $fileAttachment);
				}
