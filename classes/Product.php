...
class ProductCore extends ObjectModel
{
    ...
    /** @var int individual markuip on/off 0 - off, 1 - on */
	public $ind_markup = 0;
	...
	public function getFields()
	{
        ...
        $fields['markup'] = $this->markup;
        $fields['ind_markup'] = $this->ind_markup;
	...
	*/
	public static function getProducts($id_lang, $start, $limit, $orderBy, $orderWay, $id_category = false, $only_active = false)
	{
		...
		SELECT p.*, pl.* , t.`rate` tax_rate, m.`name` manufacturer_name, s.`name` supplier_name
		FROM `'._DB_PREFIX_.'product` p
	...
	*/
	public function deleteProductAttributes()
	{
		...
		$result &= Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'product_attribute_image`
		WHERE `id_product_attribute` IN (SELECT `id_product_attribute` FROM `'._DB_PREFIX_.'product_attribute` WHERE `id_product` = '.(int)$this->id.')');
	...
	*/
	public static function getPricesDrop($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL, $beginning = false, $ending = false)
	{
		...
		{
			$sql = '
			SELECT COUNT(DISTINCT p.`id_product`) nb
			FROM `'._DB_PREFIX_.'product` p
			WHERE p.`active` = 1
			AND p.`show_price` = 1
	...
	**/
	public static function priceCalculation($id_shop, $id_product, $id_product_attribute, $id_country, $id_state, $id_county, $id_currency, $id_group, $quantity, $use_tax, $decimals, $only_reduc, $use_reduc, $with_ecotax, &$specific_price, $use_groupReduction)
	{
		...
		if (!isset(self::$_pricesLevel2[$cacheId2]))
			...
			SELECT p.`price`, p.`wholesale_price`, p.`markup`, p.`ind_markup`,
    ...
    if (!$specific_price || $specific_price['price'] == 0) {
        ...
        $params = self::calculatePriceParams($result['wholesale_price'], $result['markup'], (bool)$result['ind_markup']);
        $price = $params['price'];
    }
	...
	*/
	...
	{
		return Db::getInstance()->ExecuteS('
		SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` group_name, agl.`public_name` public_group_name, a.`id_attribute`, al.`name` attribute_name,
	...
	*/
	public function getAccessories($id_lang, $active = true)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`, p.`upc`,
		i.`id_image`, il.`legend`, t.`rate`, m.`name` manufacturer_name, cl.`name` category_default, DATEDIFF(p.`date_add`, DATE_SUB(NOW(),
	...
	*/
	...
	{
		$result = Db::getInstance()->ExecuteS('
		SELECT p.`id_product`, pl.`name`, p.`active`, p.`reference`, m.`name` manufacturer_name
		FROM `'._DB_PREFIX_.'category_product` cp
		LEFT JOIN `'._DB_PREFIX_.'product` p ON (p.`id_product` = cp.`id_product`)
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = '.(int)($id_lang).')
		LEFT JOIN `'._DB_PREFIX_.'manufacturer` m ON (m.`id_manufacturer` = p.`id_manufacturer`)
		...
		foreach ($result as $row)
		{
			...
			if ($result2['custom'])
			{
				...
				$max_fv = Db::getInstance()->getRow('
					SELECT MAX(`id_feature_value`) nb
					FROM `'._DB_PREFIX_.'feature_value`');
	...
	*/
	...
	public function addStockMvt($quantity, $id_reason, $id_product_attribute = null, $id_order = null, $id_employee = null, $quantity_ordered = 0, $qty_ordered_old = 0)
	{
        ...
        $stockMvt->quantity_ordered = (int)$quantity_ordered;
		...
		if ($stockMvt->add(true, false, true, $qty_ordered_old))
		{
	...
	{
		return Db::getInstance()->ExecuteS('
		SELECT sm.id_stock_mvt, sm.date_add, sm.quantity, sm.quantity_ordered, sm.in_stock, sm.id_order, CONCAT(pl.name, \' \', GROUP_CONCAT(IFNULL(al.name, \'\'), \'\')) product_name, CONCAT(e.lastname, \' \', e.firstname) employee, mrl.name reason
	...
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT pl.`id_lang`, pl.`link_rewrite`, p.`ean13`, cl.`link_rewrite` category_rewrite
		FROM `'._DB_PREFIX_.'product` p
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product`)
	...
	*/
	...
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SELECT `id_category` id FROM `'._DB_PREFIX_.'category_product` WHERE `id_product` = '.(int)$this->id);
	}
	...
	*/
	...
	{
		return Db::getInstance()->ExecuteS('SELECT `id_product_attribute` id FROM `'._DB_PREFIX_.'product_attribute` WHERE `id_product` = '.(int)$this->id);
	}
	...
	*/
	public function setWsCombinations($combinations)
	{
		...
		$ids_orig = array();
		$original = Db::getInstance()->ExecuteS('SELECT `id_product_attribute` id FROM `'._DB_PREFIX_.'product_attribute` WHERE `id_product` = '.(int)$this->id);
		if (is_array($original))
			foreach ($original as $id)
				$ids_orig[] = $id['id'];

		$all_ids = array();
		$all = Db::getInstance()->ExecuteS('SELECT `id_product_attribute` id FROM `'._DB_PREFIX_.'product_attribute`');
		if (is_array($all))
			foreach ($all as $id)
				$all_ids[] = $id['id'];
		...
		foreach ($toAdd as $id)
		{
			...
			if (in_array($id, $all_ids))
				Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'product_attribute` SET id_product = '.(int)$this->id.' WHERE id_product_attribute = '.(int)$id);
			else
	...
	*/
	...
	{
		return Db::getInstance()->ExecuteS('
		SELECT DISTINCT pac.id_attribute id
		FROM `'._DB_PREFIX_.'product_attribute` pa
		LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac ON (pac.id_product_attribute = pa.id_product_attribute)
		WHERE pa.id_product = '.(int)$this->id);
	}
	...
	*
	* @return int if position exists or empty string otherwise
	*/
	...
	*/
	...
	{
		Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'product` SET ecotax = 0');
	}
	...
	public function setGroupReduction()
	{
		...
		if (!$row && !GroupReduction::deleteProductReduction((int)$this->id)) // Remove
			return false;
		elseif (!GroupReduction::setProductReduction((int)$this->id, $row['id_group'], (int)$this->id_category_default, (float)$row['reduction']))
			return false;
		return true;
	}
    ...
    /**
     * get params new price fix
     * @param type $wholesale_price
     * @param type $markup
     * @param type $ind_markup
     * @return params
     * 
     * 
     */
    public static function calculatePriceParams($wholesale_price, $markup = 0, $ind_markup = false,$id_product = null)
	{
	   
       
        
        if($ind_markup === 'false' OR $ind_markup === false) {
            $markup = _PS_SHOP_MARKUP_ / 100 + 1;
        }
        elseif($ind_markup === 'true' OR $ind_markup === true) {
            $markup = $markup / 100 + 1;
        }
        
        $wholesale_price = $wholesale_price * _PS_DOLLAR_EXCHANGE_RATE_;
        $price = $wholesale_price * $markup;
        $price = ceil($price);
  
        
        if ($price > 50 AND $price < 500) {
            $params['round'] = 5;
            $price = self::priceRound(1, 5, $price);
        }
        elseif($price >= 500 ) {
            $params['round'] = 10;
            $price = self::priceRound(1, 10, $price);
        }
        else {
            $params['round'] = 1;
        }
        
        $markupEnd = $price - $wholesale_price;
        
        
        if($id_product != null){
            
        $isSpecific =  Db::getInstance()->ExecuteS('
		SELECT *
		FROM `'._DB_PREFIX_.'specific_price`
		WHERE `id_product` = '.(int)$id_product);
        $taxRate = TaxRulesGroup::getTaxesRate(0, _PS_COUNTRY_DEFAULT_, 0, 0);
        //print_r($isSpecific);
          if(isset($isSpecific[0])){
             if($isSpecific[0]['reduction_type'] == 'amount'){
                $SpecialPrice = $price - $isSpecific[0]['reduction'];
                $markupEnd = $SpecialPrice - $wholesale_price;
             }else{
                $priceEnd = Tools::ps_round(($isSpecific[0]['reduction_type'] == 'amount') ? ($price - $isSpecific[0]['reduction'] / (1 + $taxRate / 100)) : ($price - Tools::ps_round($price * $isSpecific[0]['reduction'], 2)),2);
                $SpecialPrice = $priceEnd - $isSpecific[0]['reduction'];
                $markupEnd = $SpecialPrice - $wholesale_price;
             }
          }
        }
        ...
        $params['price'] = $price;
        $params['markup'] = $markupEnd;
        $params['wholesale_price'] = $wholesale_price;
        
		return $params;
	}
    
    /* 
    //Старая функция без подсчета не учитывая скидку.
    public function calculatePriceParams($wholesale_price, $markup = 0, $ind_markup = false)
	{
        ...
        $params['price'] = $price;
        $params['markup'] = $price - $wholesale_price;
        $params['wholesale_price'] = $wholesale_price;
    ...
    */
     ...
     * @return round price
     */
    public function priceRound ($end, $round, $price) {
        
        $tmp = substr($price, -$end);
        if ($tmp != $round) {
     ...
     * @param id_product $id
     */
    public function getAverageMonthlySales ($id) {
        
        $query = 'SELECT UNIX_TIMESTAMP(`date_add`) AS date_add, `id_stock_mvt`, `id_stock_mvt_reason`, `in_stock` 
         ...
         if ($data[0]['in_stock'] > 0) {
             $time_to = mktime(date("H"), date("i"), date("s"), date("m"), date("d"), date("Y"));
         }
         else {
             $time_to = $data[0]['date_add'];
         }
         foreach ($data AS $item) {
             if ($item['in_stock'] > 0) {
                     $time += $time_to - $item['date_add'];
             }
             $time_to = $item['date_add'];
         }
         $days = $time / 86400;
         $query_sales = 'SELECT (ABS(SUM(IFNULL(`quantity`, 0)))) AS sales 
                    FROM `'._DB_PREFIX_.'stock_mvt`
                    WHERE `id_product` = '.(int)$id.' AND `id_stock_mvt_reason` = '._STOCK_MOVEMENT_ORDER_REASON_.'
                    LIMIT 1';
         $data_sales = Db::getInstance()->ExecuteS($query_sales);
         $sales = (float)$data_sales[0]['sales'];
         if ($days < 30 ) {
             $days = 30;
         }
         $monthly_sales = $sales / $days * 30;
         
         return Tools::ps_round($monthly_sales, 2);
