...
abstract class AdminTabCore
{
	...
	protected $identifiersDnd = array('id_product' => 'id_product', 'id_category' => 'id_category_to_move','id_cms_category' => 'id_cms_category_to_move', 'id_cms' => 'id_cms', 'id_carrier' => 'id_carrier');
	...
	public function __construct()
	{
		...
		if (!$this->identifier)
			$this->identifier = 'id_'.(is_array($this->table) ? $this->table[0] : $this->table);
		...
		elseif ((Tools::getValue('submitAdd'.$this->table) AND sizeof($this->_errors)) OR isset($_GET['add'.$this->table]))
		{
			if ($this->tabAccess['add'] === '1')
			{
				...
				if ($this->tabAccess['view'])
					echo '<br /><br /><a href="'.((Tools::getValue('back')) ? Tools::safeOutput(Tools::getValue('back')) : $currentIndex.'&token='.$this->token).'"><img src="../img/admin/arrow2.gif" alt="" /> '.((Tools::getValue('back')) ? $this->l('Back') : $this->l('Back to list')).'</a><br />';
			}
			else
			...
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
				{
					if ($object->toggleStatus())
					{
						...
						{
							if (isset($object->id_parent) && $object->id_parent)
								$id_category = (int)$object->id_parent;
						}
						
						Tools::redirectAdmin($currentIndex.'&conf=5'.($id_category ? '&id_category='.(int)$id_category : '').'&token='.$token);
					}
					else
				...
				if ($value != NULL AND !strncmp($key, $this->table.'Filter_', 7 + Tools::strlen($this->table)))
				{
					...
					if ($field = $this->filterToField($key, $filter))
					{
						...
						else
						{
                            ...
                            if($key == 'a.`id_carrier`'){
                                $type = 'carselect';
                                $string_carrier = Carrier::getCarierIds($value);
                                //echo $string_carrier;
                                $string_carrier = '('.$string_carrier.')';
                            }
							...
							elseif ($type == 'carselect')
						     $sqlFilter .= (($key == $this->identifier OR $key == '`'.$this->identifier.'`') ? 'a.' : '').pSQL($key).' IN  '.pSQL($string_carrier).' ';
							else
	...
	public function getList($id_lang, $orderBy = null, $orderWay = null, $start = 0, $limit = null)
	{
		...
		$sql = 'SELECT SQL_CALC_FOUND_ROWS
			...
			WHERE 1 '.(isset($this->_where) ? $this->_where.' ' : '').($this->deleted ? 'AND a.`deleted` = 0 ' : '').(isset($this->_filter) ? $this->_filter : (isset($this->_where_default) ? $this->_where_default : '')).'
         ...
         if (!($this->_lists = Db::getInstance()->ExecuteS($sql2))){
	...
	public function displayListHeader($token = null)
	{
        ...
        $tokenAssembly = Tools::getAdminToken('AdminAssembly'.(int)(Tab::getIdFromClassName('AdminAssembly')).(int)$cookie->id_employee);     
	  echo "<script>
        $(document).ready(function() {
        
        $('.assembly').click(function() {
            var url = '/adminka/index.php?tab=AdminAssembly&token=$tokenAssembly'
         $('#orderAssembly').attr('action',url);  //заменяем в форме атрибут action
         $('#orderAssembly').attr('target','_blank');
        $('#orderAssembly').submit();  //отправляем форму
             return false;  
         });
         
         $('.filtrs').click(function() {
            var url = '$currentIndex";	if (Tools::getIsset($this->identifier))
        ...
        echo "
         $('#orderAssembly').attr('action',url);  //заменяем в форме атрибут action
         $('#orderAssembly').attr('target','_self');
		...
		echo '#'.$this->table.'"';
         ...
         echo 'class="form" id="'.$this->table.'Assembly">
		...
		echo '
                   ...
                   if($this->table == 'order'){
                     echo   '<input type="submit" name="submitAssembly'.$this->table.'" value="'.$this->l('Сборка').'" class="button assembly" />&nbsp;&nbsp;&nbsp;';
				   }		
                     echo   '<input type="submit" name="submitReset'.$this->table.'" value="'.$this->l('Reset').'" class="button reset" />
						<input type="submit" id="submitFilterButton_'.$this->table.'" name="submitFilter" value="'.$this->l('Filter').'" class="button filtrs" />
					</span>
		...
		if ($this->_list)
		{
			...
			foreach ($this->_list as $k => $tr)
			{
				...
				foreach ($this->fieldsDisplay as $key => $params)
				{
					...
					if (!isset($params['position']) AND (!isset($this->noLink) OR !$this->noLink) AND $params['noLink'] !== '1')
					...
					elseif ($params['collect'] == '1')
                        echo ' onclick="setOrderCollection('.$tr['id_order'].', '.$tr[$key].');">';
                    else
						echo '>';
					...
					elseif (isset($params['position']))
					{
						if ($this->_orderBy == 'position' AND $this->_orderWay != 'DESC')
						{
                                                    if ($this->identifier == 'id_carrier') {
                                                        ...
                                                        '&id_carrier='.(int)($tr[$this->identifier]).
                    ...
                    elseif (isset($params['collect']) AND isset($tr[$key]))
						echo '<span id="collect'.$tr['id_order'].'" class="order_collect '.($tr[$key] == '1' ? 'collect_active' : '').'"></span>';
					elseif (isset($tr[$key]))
					{
	...
	protected function _displayDuplicate($token = null, $id)
	{
		...
		echo '
			<a class="pointer" onclick="if (confirm(\''.$_cacheLang['Copy images too?'].'\')) document.location = \''.$duplicate.'&token='.($token ? $token : $this->token).'\'; else document.location = \''.$duplicate.'&noimage=1&token='.($token ? $token : $this->token).'\';">
	...
	protected function _displayEditLink($token = null, $id)
	{
		...
		echo '
			<a href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&update'.$this->table.'&token='.($token ? $token : $this->token).'">
	...
	protected function _displayDeleteLink($token = null, $id)
	{
		...
		echo '
			<a href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&delete'.$this->table.'&token='.($token ? $token : $this->token).'" onclick="return confirm(\''.$_cacheLang['DeleteItem'].$id.' ?'.
