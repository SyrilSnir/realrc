...
class OrderCore extends ObjectModel
{
    ...
    /** @var float Products total markup tax included */
	public 		$total_products_markup;
    
    /** @var float order collected */
	public 		$order_collected;

	/** @var float Shipping total */
	public 		$total_shipping;
    
    /** @var float cash_on_delivery */
	...
	public 		$date_upd;
    /** остаток от скидки */
    public  	$residue_discount;
    
    /** статус при оформлении заказа  */
    public  	$sdiscount;
	...
	public function getFields()
	{
        ...
        $fields['total_products_markup'] = (float)($this->total_products_markup);
        $fields['residue_discount'] = (float)($this->residue_discount);
        $fields['sdiscount'] = (int)($this->sdiscount);
        $fields['order_collected'] = (int)($this->order_collected);
		$fields['total_shipping'] = (float)($this->total_shipping);
        $fields['cash_on_delivery'] = (float)($this->cash_on_delivery);
   ...
   */
	...
	public function getVirtualProducts()
	{
		...
		FROM `'._DB_PREFIX_.'order_detail` od
		WHERE od.`id_order` = '.(int)$this->id.' AND `download_hash` <> \'\'');
	}
	...
	*/
	public function isVirtual($strict = true)
	{
		...
		foreach ($products as $product)
		{
			...
			if ($pd && Validate::isUnsignedInt($pd) && $product['download_hash'])
			{
				if ($strict === false)
					return true;
			}
	...
	public function getDiscountsIds($id,$details = false)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT *
		FROM `'._DB_PREFIX_.'order_discount` od '.
	...
	public static function getDiscountsCustomer($id_customer, $id_discount)
	{
		...
		WHERE o.id_customer = '.(int)$id_customer.' AND od.id_discount = '.(int)$id_discount);
	}
	...
	*/
	...
	{
		return (int)Db::getInstance()->getValue('SELECT MAX(`invoice_number`) `invoice_number` FROM `'._DB_PREFIX_.'orders`');
	}
	...
	public function setInvoice()
	{
		...
		if ($number)
			...
			$number = '(SELECT `invoice_number`
						FROM (
							SELECT MAX(`invoice_number`) + 1 `invoice_number`
							FROM `'._DB_PREFIX_.'orders`)
						tmp )';
	...
	*/
    ...
    public function setOrderCollection($order_id, $order_collected)
	{
        $collect = $order_collected === 'true' ? 0 : 1;
		if (Db::getInstance()->Execute('
				UPDATE `'._DB_PREFIX_.'orders`
                SET `order_collected` = \''.$collect.'\'
				WHERE `id_order` = '.$order_id)) {
            $return = array('collect' => true);
        }
        else {
            $return = array('error' => 'Error update order_collected');
        }
