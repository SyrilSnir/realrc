...
class FrontControllerCore
{
	...
	public function init()
	{
		...
		$smarty->assign(array(
            ...
            'TAB_MODE' => (int)Configuration::get('PRODUCT_COMMENTS_DESC'),
			
			/* Deprecated */
			'id_currency_cookie' => (int)$currency->id,
			'logged' => $cookie->isLogged(),
			'customerName' => ($cookie->logged ? $cookie->customer_firstname.' '.$cookie->customer_middlename.' '.$cookie->customer_lastname : false)
		));
	...
	public function displayHeader()
	{
		...
		self::$smarty->assign(array(
			...
			'content_only' => (int)Tools::getValue('content_only'),
                        'is_ad' => Configuration::get('PS_ISAD'),
                        'ad_message' => Configuration::get('PS_AD_MESSAGE')
		));
		self::$smarty->assign(array(
			'HOOK_HEADER' => Module::hookExec('header'),
			'HOOK_TOP' => Module::hookExec('top'),
			'HOOK_LEFT_COLUMN' => Module::hookExec('leftColumn'),
            'HOOK_MODAL' => Module::hookExec('modal')
		));
		...
		if ((Configuration::get('PS_CSS_THEME_CACHE') || Configuration::get('PS_JS_THEME_CACHE')) && is_writable(_PS_THEME_DIR_.'cache'))
		{
			...
			if (Configuration::get('PS_JS_THEME_CACHE'))
             ...
             //   if (get_class($this) == 'OrderController') {
           //         Tools::addJS(array(_PS_JS_DIR_.'jquery/jquery-3.1.0.min.js',_PS_JS_DIR_.'order.js','https://api-maps.yandex.ru/2.1/?lang=ru_RU'));
           //     }
