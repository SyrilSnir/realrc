...
class CartCore extends ObjectModel
{
	...
	public function getProducts($refresh = false, $id_product = false)
	{
		...
		p.`quantity`, p.`wholesale_price`, p.`price`,p.`markup`, p.`weight`, p.`width`, p.`height`, p.`depth`, p.`out_of_stock`, p.`active`, p.`date_add`, p.`date_upd`, IFNULL(pa.`minimal_quantity`, p.`minimal_quantity`) minimal_quantity,
		...
		foreach ($result as $row)
		{
            ...
            $markup = Product::calculatePriceParams($row['wholesale_price'], $row['markup'], true, $row['id_product']);
            $row['product_markup'] = $markup['markup'] * (int)($row['cart_quantity']);
	...
	*/
	public function getOrderTotal($withTaxes = true, $type = Cart::BOTH)
	{
		...
		foreach ($products as $product)
		{
            ...
            $this->total_products_markup += $product['product_markup']; 
		}
	...
	*/
	static function getNonOrderedCarts($dateFrom, $dateTo)
	{
		...
		SELECT cart.`id_cart`, cart.`date_upd`, c.`id_customer` id_customer, c.`lastname` customer_lastname, c.`firstname` customer_firstname, c.`middlename` customer_middlename,
