...
class ParentOrderControllerCore extends FrontController
{
	...
	protected function _assignSummaryInformations()
	{
        ...
        $sdiscount = Sdiscount::getSdiscountInfo((int)(self::$cart->id_customer));
        $my_discount = '';
        if(is_array($sdiscount)){
          
          //print_r($summary);  
          $minusSumm = Sdiscount::getPercent($summary['total_products_wt'],$sdiscount['real']); 
          $statusUserDisc = Sdiscount::getSdicount($sdiscount['real']);
          //echo $minusSumm;
          //$summary['total_price'] = $summary['total_price'] - $minusSumm;
          $summary['total_price_without_tax'] = $summary['total_price_without_tax'] - $minusSumm;
          //$summary['total_products_wt'] = $summary['total_products_wt'] - $minusSumm;
          $summary['total_discounts']  = $summary['total_discounts'] + $minusSumm;  
          $summary['total_discounts_tax_exc']  = $summary['total_discounts_tax_exc'] + $minusSumm;
          $my_discount = 'Скидка ' . '('.$statusUserDisc['name_sdiscount'].' – '.$statusUserDisc['percent_sdiscount'].'%):';
	...
	protected function _assignAddress()
	{
		...
		elseif (!Customer::getAddressesTotalById((int)(self::$cookie->id_customer)))
			Tools::redirect('address.php?back=order.php?step=2');
