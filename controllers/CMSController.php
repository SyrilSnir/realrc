	...
	public function preProcess()
	{
		...
		elseif (Validate::isLoadedObject($this->cms_category))
			...
			Tools::display404Error('404.php');
                ...
                $delivery = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT DISTINCT c.`id_carrier`, dl.`price` 
		FROM `'._DB_PREFIX_.'delivery` dl, `'._DB_PREFIX_.'carrier` c 
                WHERE dl.`id_carrier`= c.`id_carrier` AND dl.`id_zone`= 1 AND c.`active`=1 AND c.`deleted`=0');
                $pickupList = array();
                foreach ($pickups as $pickup)
                {
                    $price = retValFromArray($delivery,array('id_carrier' => $pickup['id_carrier']) , 'price');
                    $fname = _PS_IMG_DIR_.'pk/'.$pickup['id_pickup'].'.jpg';
                    if (file_exists($fname)) {
                        $pickup['img'] = _PS_BASE_URL_.'/img/pk/'.$pickup['id_pickup'].'.jpg';      
                    }
                    if ($price) {
                        $pickup['price'] = intval($price);
                    }
                    $pickupList[] = $pickup;
                }
                $this->pickupList = json_encode($pickupList);
                
                

		// too early if in setMedia
		if ($this->assignCase == 1) {
                        Tools::addJS('https://api-maps.yandex.ru/2.1/?lang=ru_RU');
...
/* <![CDATA[ */
var pickupList = <?php echo $this->pickupList?>
/* ]]> */
</script>
<?php
        }
