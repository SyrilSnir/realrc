...
class OrderControllerCore extends ParentOrderController
{
	...
	public function process()
	{
		...
		switch ((int)$this->step)
		{
			...
			case 1:                
               ...
               //$carrierTest = new Carrier(self::$cart->id_carrier);
               //if($carrierTest->is_adress == 1 and self::$cart->id_address_delivery == 0){
				//$this->_assignAddress();
               // }else{
			...
			if (Tools::isSubmit('message'))
              ...
              if (Tools::getValue('ajax')){
                ...
                $carrierTest = new Carrier(self::$cart->id_carrier);
             //   echo $carrierTest->is_adress."ffffertererter";
                if($carrierTest->is_adress == 0){
	...
	public function displayContent()
	{
		global $currency;
                $sum = self::$cart;
                if ($this->step == 1) {
                    $total = SessionsCore::getVarSession('ORDER_TOTAL');
                    SessionsCore::setValueToVar('CARD_TOTAL_SUM',$sum->getOrderTotal( true, Cart::BOTH_WITHOUT_SHIPPING));
                    self::$smarty->assign(array(
                        ...
                        'price_for_boxberry' => $total
                    ));
                }
                //,'carry_add_percent':{'51':'50'}
	...
	protected function _assignPayment()
	{
        ...
        public function setMedia() {           
            parent::setMedia();
            Tools::addJS(_PS_JS_DIR_.'order.js');
        }
         * 
         */
}
