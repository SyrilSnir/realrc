			...
			if (!($from = trim(Tools::getValue('from'))) OR !Validate::isEmail($from))
				$this->errors[] = Tools::displayError('Invalid e-mail address');
                        elseif(!($_SESSION['secpic']==strtolower($_POST['secpic'])))
                            $this->errors[] = Tools::displayError('Код с картинки введен не верно');                        
						...
						if (Validate::isLoadedObject($order) && $order->id_customer == $customer->id)
						{
							...
							if (Tools::getValue('id_product') && isset($products[(int)Tools::getValue('id_product')]))
								$email_variables['{id_product}'] = Mail::l('Related Product:').' <b>'.$products[(int)Tools::getValue('id_product')].'</b><br /><br />';
                                        ...
                                        /*
                                        if ($contact->email != 'admin@realrc.ru') {
                                            if (Mail::Send((int)self::$cookie->id_lang, 'contact', Mail::l('Message from contact form', (int)self::$cookie->id_lang), $email_variables, $contact->email, $contact->name, $from, ((int)(self::$cookie->id_customer) ? $customer->firstname.' '.$customer->middlename.' '.$customer->lastname : ''), $fileAttachment)
