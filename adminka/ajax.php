...
if (isset($_GET['ajaxDiscountCustomers']))
{
	...
	SELECT `id_customer`, `email`, CONCAT(`lastname`, \' \', `firstname`, \' \', `middlename`) as name
	...
	OR CONCAT(`firstname`,  \' \', `middlename`, \' \', `lastname` ) LIKE "%'.pSQL($filter).'%"
	OR CONCAT(`lastname`, \' \', `firstname` ,  \' \', `middlename`) LIKE "%'.pSQL($filter).'%")').'
...
if (array_key_exists('ajaxCarrierPositions', $_POST))
{
    ...
    if (is_array($positions)) {
        ...
        {
            $pos = explode('_', $value);
            if (isset ($pos[2]) AND $pos[2] == $id_carrier) {
                $position = $key;
                break;
            }
        }
...
if(Tools::getValue('getPriceAjax') == 'true') {
    die(Tools::jsonEncode(array('params' => Product::calculatePriceParams(Tools::getValue('wholesale_price'), Tools::getValue('markup'), Tools::getValue('ind_markup'),Tools::getValue('id_product')))));
}

if(Tools::getValue('setOrderCollection') == 'true') {
    die(Tools::jsonEncode(array('params' => Order::setOrderCollection((int)Tools::getValue('order_id'), Tools::getValue('order_collected')))));
}
