...
class AdminCarts extends AdminTab
{
	...
	public function viewDetails()
	{
		...
		// display cart header
		echo '<h2>'.(($customer->id) ? $customer->firstname.' '.$customer->middlename .' '.$customer->lastname : $this->l('Guest')).' - '.$this->l('Cart #').sprintf('%06d', $cart->id).' '.$this->l('from').' '.$cart->date_upd.'</h2>';
		...
		<fieldset style="width: 400px">
			...
			if ($customer->id)
				echo '
			<a href="?tab=AdminCustomers&id_customer='.$customer->id.'&viewcustomer&token='.Tools::getAdminToken('AdminCustomers'.(int)(Tab::getIdFromClassName('AdminCustomers')).(int)($cookie->id_employee)).'"> '.$customer->firstname.' '.$customer->middlename .' '.$customer->lastname.'</a></span> ('.$this->l('#').$customer->id.')<br />
