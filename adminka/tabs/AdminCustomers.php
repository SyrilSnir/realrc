...
class AdminCustomers extends AdminTab
{
	...
	public function viewcustomer()
	{
       ...
       $addresses[0]['lastname'] = AdminCustomers::mb_ucfirst($addresses[0]['lastname']);
        $addresses[0]['middlename'] = AdminCustomers::mb_ucfirst($addresses[0]['middlename']);
        $addresses[0]['firstname'] = AdminCustomers::mb_ucfirst($addresses[0]['firstname']);
        $addresses[0]['state'] = mb_strtolower($addresses[0]['state']);
        $addresses[0]['state'] = AdminCustomers::mb_ucfirst($addresses[0]['state']);
        $addresses[0]['address1'] = AdminCustomers::mb_ucfirst($addresses[0]['address1']);
        $addresses[0]['city'] = AdminCustomers::mb_ucfirst($addresses[0]['city']);
        ...
        $customer->firstname = AdminCustomers::mb_ucfirst($customer->firstname);
        $customer->middlename = AdminCustomers::mb_ucfirst($customer->middlename);
        $customer->lastname = AdminCustomers::mb_ucfirst($customer->lastname);
        echo '
		<fieldset style="width:400px;float: left"><div style="float: right"><a href="'.$currentIndex.'&addcustomer&id_customer='.$customer->id.'&token='.$this->token.'"><img src="../img/admin/edit.gif" /></a></div>
			<span style="font-weight: bold; font-size: 14px;">'.$customer->firstname.' '.$customer->middlename .' '.$customer->lastname.'</span>
		...
		if (count($messages))
		{
			...
			echo $customer->firstname.' '.$customer->middlename .' '.$customer->lastname.' '.$this->l('has never contacted you.');
		...
		if ($orders && count($orders))
		{
			...
			echo $customer->firstname.' '.$customer->middlename .' '.$customer->lastname.' '.$this->l('has not placed any orders yet');
		...
		if (count($addresses))
		{
			...
			echo $customer->firstname.' '.$customer->middlename .' '.$customer->lastname.' '.$this->l('has not registered any addresses yet').'.';
		echo '<div class="clear">&nbsp;</div>
		...
		if (count($discounts))
		{
			...
			echo $customer->firstname.' '.$customer->middlename .' '.$customer->lastname.' '.$this->l('has no discount vouchers').'.';
		echo '<div class="clear">&nbsp;</div>';
	...
	public function displayForm($isMainTab = true)
	{
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/tab-customers.gif" />'.$this->l('Customer').'</legend>
                ...
                <label>'.$this->l('Middle name:').' </label>
				<div class="margin-form">
					<input type="text" size="33" name="middlename" value="'.htmlentities($this->getFieldValue($obj, 'middlename'), ENT_COMPAT, 'UTF-8').'" /> <sup>*</sup>
					<span class="hint" name="help_box">'.$this->l('Forbidden characters:').' 0-9!<>,;?=+()@#"�{}_$%:<span class="hint-pointer">&nbsp;</span></span>
				</div>'
                ;
		...
		echo '
				...
				<div class="margin-form">
					<input type="text" size="33" name="phone" value="'.htmlentities($this->getFieldValue($obj, 'phone'), ENT_COMPAT, 'UTF-8').'" /> <sup>*</sup>
				</div>
