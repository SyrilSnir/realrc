		...
		else
		{
			...
			if (!$searchType || $searchType == 1)
			{
				$this->fieldsDisplay['catalog'] = (array(
                    ...
                    'images' => array('title' => $this->l('Изображение')),
			...
			if (!$searchType OR $searchType == 2 OR $searchType == 6)
			{
				...
				{
                                    // отключено для реализации поиска по номеру телефона
					/* Handle customer ID */
...
$(document).ready(function() {
    $(".img_sear").fancybox();
           ...
           $image = array();
            ...
            $p_image_temp_s = isset($image['id_image']) ? __PS_BASE_URI__._PS_PROD_IMG_.$imageObj->getExistingImgPath().'-small.jpg' : '#';
            $p_image_temp_b = isset($image['id_image']) ? __PS_BASE_URI__._PS_PROD_IMG_.$imageObj->getExistingImgPath().'-thickbox.jpg' : '#';
           //echo   $productAllData->link_rewrite;       
            $p_image_small = $p_image_temp_s;
            $p_image_big = $p_image_temp_b;
				echo '
				<tr>
					<td>'.$product['id_product'].'</td>
                    <td><a class="img_sear" href="'. $p_image_big.'" ><img src="'.$p_image_small.'" /></a></td>
		...
		if (isset($this->_list['customers']) AND !empty($this->_list['customers']) AND $nbCustomers = sizeof($this->_list['customers']))
		{
			...
			foreach ($this->_list['customers'] AS $k => $customer)
			{
				...
				<tr class="'.($irow++ % 2 ? 'alt_row' : '').'">
					...
					<td>'.Tools::safeOutput($customer['lastname']).' '.Tools::safeOutput($customer['firstname']) .' '.Tools::safeOutput($customer['middlename']).'</td>
