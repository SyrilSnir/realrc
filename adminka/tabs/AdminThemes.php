...
class AdminThemes extends AdminPreferences
{
		...
		$this->table = 'configuration';
 		...
 		$this->_fieldsAppearance = array(
			...
			'PS_NAVIGATION_PIPE' => array('title' => $this->l('Navigation pipe:'), 'desc' => $this->l('Used for navigation path inside categories/product'), 'cast' => 'strval', 'type' => 'text', 'size' => 20),
                        'PS_ISAD' => array('title' => "Отображать информационное сообщение", 'desc' => "Показывать информационное сообщение в шапке сайта", 'validation' => 'isBool', 'cast' => 'intval', 'required' => true, 'type' => 'bool'),
                        'PS_AD_MESSAGE' => array('title' => "Текст информационного сообщение для шапки", 'desc' => "Добавить текст информационного сообщения", 'cast' => 'strval', 'type' => 'textarea', 'cols' => 50, 'rows' => 10),
		);
	...
	private function _getThemesList()
	{
		...
		while ($folder = readdir($dir))
			if ($folder != '.' && $folder != '..' && is_dir(_PS_ALL_THEMES_DIR_.$folder) && file_exists(_PS_ALL_THEMES_DIR_.'/'.$folder.'/preview.jpg'))
