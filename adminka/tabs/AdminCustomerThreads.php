...
class AdminCustomerThreads extends AdminTab
{
	public function __construct()
	{
 		...
 		$this->_select = 'CONCAT(c.firstname," ",c.middlename," ",c.lastname) as customer, cl.name as contact, l.name as language, COUNT(cm.`id_customer_message`) messages, (
	...
	public function postProcess()
	{
		...
		if ($id_customer_thread = (int)Tools::getValue('id_customer_thread'))
		{
			...
			if (isset($_POST['id_employee_forward']))
			{
				...
				SELECT ct.*, cm.*, cl.name subject, CONCAT(e.firstname, \' \', e.lastname) employee_name, CONCAT(c.firstname, \' \', c.middlename, \' \', c.lastname) customer_name, c.firstname
		...
		if ($thread->id_customer)
		{
			...
			if ($orders AND sizeof($orders))
			{
				...
				if ($countOK = sizeof($ordersOK))
				{
					...
					echo '</table>
					<h3 style="color:green;font-weight:700;margin-top:10px">'.$this->l('Validated Orders:').' '.$countOK.' '.$this->l('for').' '.Tools::displayPrice($totalOK, new Currency(_PS_CURRENCY_DEFAULT_)).'</h3>
					</div>';
				}
			}
