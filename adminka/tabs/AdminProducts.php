...
class AdminProducts extends AdminTab
{
	...
	protected function copyFromPost(&$object, $table)
	{
        ...
        if(isset($_POST['ind_markup']) AND $_POST['ind_markup'] == 'on') {
            $object->markup = (int)$_POST['markup'];
            $object->ind_markup = 1;
        }
        else {
            $object->ind_markup = 0;
            $object->markup = (int)_PS_SHOP_MARKUP_;
        }
	}
			...
			if (isset($id) AND !empty($id))
			{
				...
				if (Validate::isLoadedObject($object))
				{
					...
					if ($object->update())
					{
						...
						{
							$reason = new StockMvtReason((int)$id_reason);
                            $qty_ordered_old = Tools::getValue('quantity_ordered');
                            if ($reason->sign == '1' OR $reason->sign == '-1') {
                                $qty = Tools::getValue('mvt_quantity') * $reason->sign;
                                $quantity_ordered = 0;
                            }
                            elseif ($reason->sign == '2') {
                                $qty = 0;
                                $quantity_ordered = Tools::getValue('mvt_quantity') * 1;
                            }
                            elseif ($reason->sign == '-2') {
                                $qty = 0;
                                $quantity_ordered = Tools::getValue('mvt_quantity') * -1;
							...
							if (!$object->addStockMvt($qty, (int)$id_reason, NULL, NULL, (int)$cookie->id_employee, $quantity_ordered, $qty_ordered_old))
								$this->_errors[] = Tools::displayError('An error occurred while updating qty.');
						}
	...
	public function displayForm($isMainTab = true)
	{
		...
		<script type="text/javascript">
			...
			function updateMvtStatus(id_mvt_reason)
			{
                ...
                else if ($(\'#id_mvt_reason option:selected\').attr(\'rel\') == -2)
                    $(\'#mvt_sign\').html(\'<img src="../img/admin/arrow_down.png" /> '.$this->l('Decrease your stock ordered').'\');
                else if ($(\'#id_mvt_reason option:selected\').attr(\'rel\') == 2)
                    $(\'#mvt_sign\').html(\'<img src="../img/admin/arrow_up.png" /> '.$this->l('Increase your stock ordered').'\');
				else
	...
	function displayFormInformations($obj, $currency)
	{
		...
		echo '		</td>
                    ...
                    <tr>
						<td class="col-left">'.$this->l('Multiplicity (packed):').'</td>
						<td style="padding-bottom:5px;">
							<input size="6" maxlength="6" name="packed" type="text" value="'.htmlentities($this->getFieldValue($obj, 'packed'), ENT_COMPAT, 'UTF-8').'" onKeyUp="if (isArrowKey(event)) return ;this.value = this.value.replace(/,/g, \'.\');" /> '.$this->l('pcs').'
						</td>
					</tr>
				</table>
...
: '' ) : htmlentities(Tools::getValue('virtual_product_expiration_date'), ENT_COMPAT, 'UTF-8') ?>" size="11" maxlength="10" autocomplete="off" /> <?php echo $this->l('Format: YYYY-MM-DD'); ?>
	...
	<script type="text/javascript">
        ...
        function calculatePrice() {
            ...
            //console.log(markup);
            var ind_markup = $('#ind_markup').prop("checked");
            //console.log(ind_markup);
            $.ajax({
            type: "GET",
            url: "/adminka/ajax.php",
            data: "getPriceAjax=true&token=<?php echo Tools::getValue('token') ?>&id_product=<?php echo Tools::getValue('id_product') ?>&wholesale_price="+wholesale_price+"&markup="+markup+"&ind_markup="+ind_markup,
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(msg){
                //console.log(msg.params.price);
                $('#priceTE').val(msg.params.price);
                $('#priceTI').val(msg.params.price);
                $('#span_wholesale_price').html(msg.params.wholesale_price);
                $('#markup_rub').html(msg.params.markup);
                $('#round_').html(msg.params.round);
                $('#finalPrice').html(msg.params.price);
                $('#finalPriceWithoutTax').html(msg.params.price);
            }
 });
        }
	</script>
...
<?php
$price = Product::calculatePriceParams($obj->wholesale_price, $obj->markup, (bool)$obj->ind_markup,$obj->id);

					echo '
					<tr>
						<td class="col-left">'.$this->l('Pre-tax wholesale price:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format % 2 != 0 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" name="wholesale_price" id="wholesale_price" type="text" value="'.htmlentities($this->getFieldValue($obj, 'wholesale_price'), ENT_COMPAT, 'UTF-8').'" onchange="this.value = this.value.replace(/,/g, \'.\');calculatePrice();" onkeyup="calculatePrice();"/> '.$this->l('usd').'
							<span style="margin-left:10px">'.$this->l('The wholesale price at which you bought this product').
                            ': <span style="background: #0f0;" class="bold"><span id="span_wholesale_price">'.($this->getFieldValue($obj, 'wholesale_price') * _PS_DOLLAR_EXCHANGE_RATE_).'</span> '.$this->l('rub.').'</span></span>
						</td>
					</tr>';
                    echo '
                    <tr class="bold">
                        <td class="col-left">'.$this->l('Dollar exchange rate').':</td>
                        <td style="padding-bottom:5px;">
                            '._PS_DOLLAR_EXCHANGE_RATE_.' '.$this->l('rub.').'
                        <span style="margin-left: 40px;">
                            '.$this->l('Surcharge default').':
                        </span>
                        <span style="margin-left: 40px;">
                            '._PS_SHOP_MARKUP_.'%
                        </span>
                    ...
                    echo '
                    <tr class="bold">
                        <td class="col-left">'.$this->l('Margin (in %)').':</td>
                        <td style="padding-bottom:5px;">
                            <input size="11" maxlength="14" name="markup" id="markup" type="text" value="'.
                            (($obj->markup == '0' OR $obj->ind_markup == '0') ? _PS_SHOP_MARKUP_ : htmlentities($this->getFieldValue($obj, 'markup'), ENT_COMPAT, 'UTF-8')).
                            '" onchange="this.value = this.value.replace(/,/g, \'.\');calculatePrice();" onkeyup="calculatePrice();"/>
                        <span style="margin-left: 40px;">
                            '.$this->l('Surcharge in rubles').': <span style="background: #ff0;"><span id="markup_rub">'.$price['markup'].'</span> '.$this->l('rub.').'</span>
                        </span>
                        <span style="margin-left: 40px;"><input type="checkbox" name="ind_markup" id="ind_markup" '.($obj->ind_markup == '1' ? 'checked ' : '').' onchange="calculatePrice();" /></span>
                        <span style="margin-left: 20px;">-
                            '.$this->l('use individual markup').'
                        </span>
                        </td>
                    </tr>';
                    echo '
                    <tr class="bold">
                        <td class="col-left">'.$this->l('Round up (rub.)').':</td>
                        <td style="padding-bottom:5px;">
                            <span id="round_">'.$price['round'].'</span>'.
                        '</td>
                    </tr>';
					echo '
					<tr>
						...
						<td style="padding-bottom:5px;">
							'.($currency->format % 2 != 0 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="priceTE" name="price" type="text" value="'.htmlentities($price['price']/*$this->getFieldValue($obj, 'price')*/, ENT_COMPAT, 'UTF-8').'" onchange="this.value = this.value.replace(/,/g, \'.\');" onkeyup="if (isArrowKey(event)) return; calcPriceTI();" />'.($currency->format % 2 == 0 ? ' '.$currency->sign : '').'<sup> *</sup>
				...
				if ((int)Configuration::get('PS_STOCK_MANAGEMENT'))
				{
					...
					if (!$has_attribute)
					{
						if ($obj->id)
						{
							...
							foreach ($reasons AS $reason) {
								if ($reason['id_stock_mvt_reason'] == 3 OR $reason['id_stock_mvt_reason'] == 4) {
                                    continue;
                                }
                    ...
                    $qty_ordered = $this->getFieldValue($obj, 'quantity_ordered');
					if ($obj->id)
						echo '
							<tr><td class="col-left">'.$this->l('Quantity in stock:').'</td>
								<td style="padding-bottom:5px;"><b>'.$qty.'</b><input type="hidden" name="quantity" value="'.$qty.'" />
                            ...
                            <tr><td class="col-left">'.$this->l('Quantity ordered:').'</td>
								<td style="padding-bottom:5px;"><b>'.$qty_ordered.'</b><input type="hidden" name="quantity_ordered" value="'.$qty_ordered.'" /></td>
							</tr>
						';
                        
                   $monthly_sales = Product::getAverageMonthlySales($this->getFieldValue($obj, 'id'));
                    if((int)$qty_ordered < 0){
                        $qty_orderednum = 0; 
                    }else{
                        $qty_orderednum = $qty_ordered;
                    }

                    $kratnost = htmlentities($this->getFieldValue($obj, 'packed'), ENT_COMPAT, 'UTF-8');
                   ...
                   // echo $qty;
                    ...
                    echo '<tr><td class="col-left">'.$this->l('Дозаказ:').'</td>
								<td style="padding-bottom:5px;"><b>'.$reorder.'</b></td>
							</tr>';
                    $allordereds =  Db::getInstance()->ExecuteS('
                            		SELECT quantity
                            		FROM `'._DB_PREFIX_.'stock_mvt`
                            		WHERE `id_product` = '.(int)$obj->id.' and id_stock_mvt_reason = 3 ');
                    ...
                    echo '<tr><td class="col-left">'.$this->l('Продано:').'</td>
								<td style="padding-bottom:5px;"><b>'.$allordered.'</b></td>
							</tr>';         
                    $monthly_sales = Product::getAverageMonthlySales($this->getFieldValue($obj, 'id'));
                    echo '<tr><td class="col-left">'.$this->l('Average monthly sales:').'</td>
								<td style="padding-bottom:5px;"><b>'.$monthly_sales.'</b></td>
							</tr>';
                  // print_R($obj); 
                  $tokenMovement = Tools::getAdminToken('AdminStockMvt'.(int)(Tab::getIdFromClassName('AdminStockMvt')).(int)$cookie->id_employee);     
                  //$obj->name[6]
                   $MovementId = Db::getInstance()->getValue('
                                                                        SELECT id_stock_mvt
                                                                		FROM `'._DB_PREFIX_.'stock_mvt`
                  ...
                  <td class="col-left">
                  <br>
                 <a class="button" href="/adminka/index.php?tab=AdminStockMvt&token='.$tokenMovement.'&id_stock_mvt='.$MovementId.'&viewstock_mvt" target="_blank">Склад</a>
                    </td>
...
$(document).ready(function() {
    $(".img_cat").fancybox();
						...
						foreach ($images AS $k => $image)
						{
							...
							<tr>
								<td style="padding: 4px;"><a href="'._THEME_PROD_DIR_.$img_path.'.jpg" rel="group"  class="img_cat">
