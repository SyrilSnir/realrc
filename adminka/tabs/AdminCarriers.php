...
class AdminCarriers extends AdminTab
{
	...
	public function __construct()
	{
		...
		'is_adress' => array('title' => $this->l('Требовать ввод адреса'), 'align' => 'center', 'active' => 'status', 'type' => 'bool', 'orderby' => false));
        ...
        //getList((int)($cookie->id_lang))
  /*      public function display() 
        {
            
            $this->getList((int)($cookie->id_lang),'position','ASC');
            $this->displayList();
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/delivery.gif" alt="" />'.$this->l('Carriers').'</legend>
				...
				<label>Имя модуля: </label>
                                <div class="margin-form">
                                <input type="text" size="25" name="external_module_name" value="'.htmlentities($this->getFieldValue($obj, 'external_module_name'), ENT_COMPAT, 'UTF-8').'" />                                    
                                </div>
                                <label>Включить внешний модуль: </label>
                                <div class="margin-form">
					<input type="radio" name="is_module" id="is_module_on" value="1" '.($this->getFieldValue($obj, 'is_module') ? 'checked="checked" ' : '').'/>
					<label class="t" for="is_module_on"> <img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
					<input type="radio" name="is_module" id="is_module_off" value="0" '.(!$this->getFieldValue($obj, 'is_module') ? 'checked="checked" ' : '').'/>
					<label class="t" for="is_module_off"> <img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" /></label>					
				</div>
                                <label>Стоимость доставки из внешнего модуля: </label>
                                <div class="margin-form">
					<input type="radio" name="shipping_external" id="shipping_external_on" value="1" '.($this->getFieldValue($obj, 'shipping_external') ? 'checked="checked" ' : '').'/>
					<label class="t" for="shipping_external_on"> <img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
					<input type="radio" name="shipping_external" id="shipping_external_off" value="0" '.(!$this->getFieldValue($obj, 'shipping_external') ? 'checked="checked" ' : '').'/>
					<label class="t" for="shipping_external_off"> <img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" /></label>
					<p>Использовать внешний модуль для расчета стоимости доставки</p>
				</div>
                ...
                <label>'.$this->l('Carrier text field:').' </label>
				<div class="margin-form">
					<textarea cols="60" rows="6" name="carrier_text">'
                        .htmlentities($this->getFieldValue($obj, 'carrier_text'), ENT_COMPAT, 'UTF-8').
                    '</textarea>
					<p class="clear">'.$this->l('Carrier text field description').'</p>
				</div>
                ...
                <label>'.$this->l('Требовать ввод адреса:').' </label>
				<div class="margin-form">
					<input type="radio" name="is_adress" id="is_adress_on" value="1" '.($this->getFieldValue($obj, 'is_adress') ? 'checked="checked" ' : '').'/>
					<label class="t" for="is_adress_on"> <img src="../img/admin/enabled.gif" alt="'.$this->l('Yes').'" title="'.$this->l('Yes').'" /></label>
					<input type="radio" name="is_adress" id="is_adress_off" value="0" '.(!$this->getFieldValue($obj, 'is_adress') ? 'checked="checked" ' : '').'/>
					<label class="t" for="is_adress_off"> <img src="../img/admin/disabled.gif" alt="'.$this->l('No').'" title="'.$this->l('No').'" /></label>
				</div>	
				...
				<div class="margin-form">
					...
					<label class="t" for="billing_weight">'.$this->l('According to total weight').'</label><br />
                                        <input type="radio" name="shipping_method" id="billing_external" value="'.Carrier::SHIPPING_METHOD_EXTERNAL.'" '.($this->getFieldValue($obj, 'shipping_method') == Carrier::SHIPPING_METHOD_EXTERNAL ? 'checked="checked" ' : '').'/>
					<label class="t" for="billing_external">Расчет мз внешнего модуля</label><br />
				</div>
